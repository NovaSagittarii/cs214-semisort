...
int main(int argc, char* argv[]) {
  ...
  const size_t element_types = 10000000;
  Type* A = (Type*)malloc(n * sizeof(Type));
  parlay::parallel_for(0, n, [&](size_t j) { A[j] = {hash64(j * random_seed) % element_types, hash64(j * random_seed)}; });
  // Note: checker doesn't run well with few duplicates.
  const bool run_checker = n <= 1e7 || element_types <= 1e4;
  // [Checker] get counts
